    ...
    case TT_EXP_CONTLIST:
      {
        ...
        for(oi = this->db->object_list.begin(); oi != this->db->object_list.end(); oi++) {
          if ((*oi)->get_class_id() == TT_POINT_CMD) {
            ...
            if ((pt->type == TT_POINT_TYPE_CONTINUATION) && ((!pt->txt.empty()) || (!this->filter)) && (pt->fsptr->is_selected())) {
              this->db->db2d.process_projection(pt->fscrapptr->proj);
              this->m_table.insert_object(nullptr);
              this->m_table.insert_attribute("Comment",ths2txt(pt->txt.c_str(), layout->lang).c_str());
	            if (!thisnan(pt->xsize))
