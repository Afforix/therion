      ...
      if (! pp->station_name.is_empty()) {
        ...
        if (optr != NULL) {
          if (optr->get_class_id() == TT_SCRAP_CMD) {
            if (auto& scrap = dynamic_cast<thscrap&>(*optr);
                scrap.proj->type == TT_2DPROJ_NONE) {
              pp->scrap = &scrap;
            } else {
              extend_error = true;
...
//      else
//        ththrow("can not connect all scraps in exteded:{} projection", prj->index);      
//    }
//    
//    delete [] xscraps;
//    delete [] xstations;
